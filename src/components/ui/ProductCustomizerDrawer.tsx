...
import { X, Calendar, User, Hash, CheckCircle2, QrCode } from "lucide-react"
...
export function ProductCustomizerDrawer({ isOpen, onClose, product }: ProductCustomizerDrawerProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    customName: "",
    customNumber: "",
    pickupDate: "Hoy (Próximo Partido)"
  })
                ...
                {step === 2 && (
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      {["Hoy (Próximo Partido)", "Mañana", "Fin de Semana"].map((date) => (
                        <div 
                          key={date}
                          onClick={() => setFormData({...formData, pickupDate: date})}
                          className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${formData.pickupDate === date ? "border-accent-primary bg-accent-primary/10" : "border-zinc-800 bg-zinc-900 hover:border-zinc-700"}`}
                        >
                          <h5 className="font-bold text-white">{date}</h5>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex gap-4 pt-8">
                      ...
                      <Button onClick={handleComplete} className="w-full h-12">Confirmar Click & Collect</Button>
                    </div>
                  </motion.div>
                )}
                ...
                {step === 3 && (
                  ...
                  >
                    ...
                    <p className="text-zinc-400 text-sm">
                      Muestra este código QR en la barra el día <strong className="text-white">{formData.pickupDate}</strong> para pagar y recoger tu {product?.name}.
                    </p>
