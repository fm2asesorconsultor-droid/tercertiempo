...
import { Play, ChevronRight, Monitor, Sofa, ShoppingBag, MousePointer2, type LucideIcon } from "lucide-react"
import { getHeroContent, getExperienceCards } from "@/lib/data/hero"
...
export async function HeroSection() {
  const [hero, cards, siteSettings] = await Promise.all([
    getHeroContent(),
    getExperienceCards(),
    getSiteSettings(),
  ])
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid gap-12 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="flex flex-col justify-center space-y-8 lg:col-span-5 relative z-20">
            <FadeIn delay={0.1}>
              <p className="font-title font-black text-accent-primary text-xl tracking-wider">{hero.eyebrow}</p>
            ...
            <FadeIn delay={0.2} direction="up">
              <h2 className="text-xl md:text-2xl font-bold text-white">{hero.headline}</h2>
            ...
            <FadeIn delay={0.3} direction="up" className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="w-full sm:w-auto font-bold tracking-wide">
                {hero.ctaPrimaryLabel} <ChevronRight className="ml-2 h-5 w-5" />
              </Button>
              <Button variant="outline" size="lg" className="w-full sm:w-auto font-bold tracking-wide group">
                <Play className="mr-2 h-5 w-5 text-accent-primary group-hover:text-white" /> {hero.ctaSecondaryLabel}
              </Button>
            </FadeIn>
          </div>
          ...
          <div className="flex overflow-x-auto snap-x lg:flex-col justify-start lg:justify-center gap-4 lg:col-span-3 relative z-20 pb-4 lg:pb-0 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
            {cards.map((card, i) => {
              const Icon = ICONS[card.iconKey] ?? Monitor
              return (
                <div key={card.id} className="shrink-0 w-[85vw] sm:w-[320px] lg:w-full snap-start lg:shrink">
                  <FadeIn delay={0.4 + i * 0.1} direction="left">
                    <ExperienceCard
